...
void WaveHandler::Update() {
	...
	if (waveInProgress && zombieToSpawn > 0 && Game::Time() >= lastSpawnTick + SPAWN_DELAY) {
...
void WaveInfoUI::InitializeUI() {
	...
	wave_text->transform->position = (Game::WindowResolution() - wave_text->transform->scale) / 2.0f;
